...
class test_login:
    ...
    def test_提交订单(self):
        ...
        url_add =base_url + 'zlead/voucher/addVoucher'
        #data_add = {'amount':'11','img'='http://139.224.113.24/group1/M00/00/06/rBN_h10As6SAYuhoAAEMazy6pHc589.jpg','payType':'1','payTime':'2019/06/12','payRemar':'11','orderIds':orderId}
        data_add = {'amount':'11','img':'http://139.224.113.24/group1/M00/00/06/rBN_h10As6SAYuhoAAEMazy6pHc589.jpg','payType':'1','payTime':'2019/06/12','payRemar':'11','orderIds':orderId}
        mylogger.info("添加凭证请求数据:"+str(data_add))
        res=seesion.post(url=url_add,data=data_add,cookies=cookies)
        print(res.json())
        if "添加凭证成功" in res.json().values():
            mylogger.info("添加凭证成功")
            fail_count=0
            pass_count=1
        else:
            mylogger.info("添加凭证失败")
        content = ('''<tr><td>登录、提交订单、上传凭证流程</td><td>'''+str(1)+'''</td><td>'''+str(pass_count)+'''</td><td>'''+str(fail_count)+'''</td></tr>''')
        mylogger.info("登录、提交订单、上传凭证流程测试结果:用例总数"+str(1)+"条，其中通过用例"+str(pass_count)+"条，失败用例"+str(fail_count)+'条')
        #print(content)
