...
file_name="登录_工厂PC端.xlsx"

#http://192.168.3.214:8803/zlead/login.do?act=0.31052332965827856
url = base_url_FPC+'/zlead/login.do'
...
class test_login:
    def __init__(self,file_name=file_name,Sheet_name='test'):
        ...
        self.loginName = list(self.sheet["B"])
        self.userPwd = list(self.sheet["C"])
        self.rdmCode = list(self.sheet["D"])
    ...
    def test_登录(self):
        ...
        for i in range(1,self.nrows):
            url1 = base_url_FPC+'/zlead/isShowRdm.do?loginName='
            mylogger.info("登录URL："+url)
            res=requests.get(url =url1)
            print(res.json())
            tid = (res.json()['data'])
            mylogger.info("第"+str(i)+"条用例\""+str(self.description[i].value)+"\"开始执行")
            data={'loginName':str(self.loginName[i].value),'userPwd':md5parse(str(self.userPwd[i].value).encode()),'tid':tid,'rdmCode':''}
            ...
            s=requests.session()
            mylogger.info(list(p.json().values())[0])
            mylogger.info((self.ex_res[i].value))
            if str(self.ex_res[i].value) in str(list(p.json().values())[0]):
